...
vector n a
  | n < 0 = error "indice no valido"
  | otherwise  = V (pot2 n) (Unif a)
...
simplify left right = Node left right

-------------------------------------------------------------------------------
-- | Exercise d.

get :: Ord p => Int -> Vector p -> p
get index vector@(V x tree)
  | index < 0 || index >= size vector = error "Indice incorrecto"
  | otherwise = get' index x tree

get' :: (Ord a) => Int -> Int -> Tree a -> a
get' index sz (Unif x) = x
get' index sz (Node lt rt)
  | index < div sz 2 = get' index (div sz 2) lt
  | otherwise = get' (index - div sz 2) (div sz 2) rt
...
set ind n (V size (Node lt rt)) = V size (rece ind n  size (Node lt rt))
  where
    rece ind n size (Unif x)
      |size == 1 = Unif n
      |ind >=0 && ind <= div size 2-1 = Node ( rece ind n (div size 2) (Unif x)) (Unif x)
      |ind >= div size 2 && ind <= size-1 = Node (Unif x) ( rece (ind - div size 2) n (div size 2)  (Unif x) )

    rece ind n size (Node lt rt)
      |ind >=0 && ind <= div size 2-1 =  simplify (rece ind n (div size 2) lt) rt--primera mitad
      |ind >= div size 2 && ind <= size-1 =  simplify lt (rece (ind - div size 2) n (div size 2) rt)--segunda mitad
      |otherwise = error "Indice no valido"
...
mapVector :: Eq a => (t -> a) -> Vector t -> Vector a
mapVector f (V sz tree) = V sz (mapV f tree)

mapV :: Eq a => (t -> a) -> Tree t -> Tree a
mapV f (Node (Unif x) (Unif y)) = simplify (Unif (f x)) (Unif (f y))
mapV f (Node l r) = simplify (mapV f l) (mapV f r) 
