...
module DataStructures.Set.AVLSet
  ( Set
  , empty
  , isEmpty
  , size
...
insert x (S tree)  = S (T.insert x tree) -- relies on AVL not admiting repeated elements
...
instance (Show a) => Show (Set a) where
  show s  = "AVLSet(" ++ intercalate "," (map show $ elems s) ++ ")"
