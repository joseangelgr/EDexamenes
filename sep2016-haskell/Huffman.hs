...
weights [] = D.empty
weights (x:xs) = weights' (x:xs) D.empty
  where
    weights' [] d = d
    weights' (x:xs) d = weights' xs (D.updateOrInsert x (+1) 1 d)
...
huffmanLeaves s
  | D.size (weights s) <= 1 = error "no hay 2 caracteres diferentes"
  | otherwise = D.foldKeysValues (\k v pq -> PQ.enqueue (WLeaf k v) pq) PQ.empty (weights s)
...
huffmanTree str = huffmanTree' (merge l1 l2) (PQ.first leaves) leaves
  where
    (l1, l2, leaves) = (PQ.first leaves_pq, PQ.first (PQ.dequeue leaves_pq), PQ.dequeue(PQ.dequeue leaves_pq))
    leaves_pq = huffmanLeaves str
    huffmanTree' wlt leaf pq
      | PQ.isEmpty (PQ.dequeue pq)= merge wlt leaf
      | otherwise= huffmanTree' (PQ.first (PQ.dequeue pq))  (merge wlt leaf) (PQ.dequeue pq)
...
joinDics d1 d2 = D.foldKeysValues joinValues joined_d2 d1
  where
    joined_d2 = D.foldKeysValues joinValues D.empty d2
    joinValues = \k v dict -> D.insert k v dict
...
prefixWith i dict = D.foldKeysValues (\k v dict -> D.insert k (i:v) dict) D.empty dict
...
huffmanCode (WNode lc rc _) = joinDics down_left down_right
  where
    down_left = prefixWith 0 (huffmanCode lc)
    down_right = prefixWith 1 (huffmanCode rc)
